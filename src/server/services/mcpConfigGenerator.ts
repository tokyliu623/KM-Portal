...
function resolveServerName(input: McpConfigInput): string {
  return input.serverName || 'vivo-knowledge'
}

function resolveAuthHeader(input: McpConfigInput): string {
  if (!input.accessToken) return `Bearer ${ACCESS_TOKEN_PLACEHOLDER}`
...
export function generateClaudeDesktopConfig(input: McpConfigInput): McpConfigResult {
...
export function generateCursorConfig(input: McpConfigInput): McpConfigResult {
  const serverName = resolveServerName(input)
  const config = {
    mcpServers: {
      [serverName]: {
...
export function generateContinueConfig(input: McpConfigInput): McpConfigResult {
  const serverName = resolveServerName(input)
  const config = {
    mcpServers: [
      {
...
export function generateClineConfig(input: McpConfigInput): McpConfigResult {
  const serverName = resolveServerName(input)
  const config = {
    mcpServers: {
      [serverName]: {
...
export function generateAllMcpConfigs(input: McpConfigInput): Record<string, McpConfigResult> {
  return {
