...
export interface ParameterObject {
  ...
  required?: boolean
  description?: string
  schema: { type: string; default?: string | number }
}
...
export interface OpenApiSpecObject {
  openapi: '3.0.3'
  info: {
    title: string
    description: string
...
export function getServers(): ServerObject[] {
  return [
    {
...
export function getComponents(): ComponentsObject {
  return {
    securitySchemes: {
      bearerAuth: {
        type: 'http',
...
function kbIdParam(): ParameterObject {
  return {
    name: 'kbId',
    in: 'path',
    required: true,
...
export function getPaths(kbId: number): Record<string, PathItemObject> {
  void kbId
  return {
...
export function generateOpenApiSpec(input: OpenApiSpecInput): OpenApiSpecObject {
