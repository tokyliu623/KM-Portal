...
async function readStore(): Promise<SkillsStore> {
  try {
...
async function writeStore(store: SkillsStore): Promise<void> {
  await fs.mkdir(DATA_DIR, { recursive: true })
  await fs.writeFile(SKILLS_FILE, JSON.stringify(store, null, 2), 'utf-8')
}

function generateSkillContent(name: string, kbId: number, kbName: string, permission: 'read' | 'write'): string {
...
router.post('/', async (req, res) => {
  try {
    ...
    const responseData: Record<string, unknown> = { ...skill }
    if (apiKey) {
      responseData.apiKey = apiKey
    }
    const responseBody: Record<string, unknown> = { success: true, data: responseData }
...
router.get('/:id/export', async (req, res) => {
  try {
    ...
    let apiKeyForZip: string | undefined
    try {
