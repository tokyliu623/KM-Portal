...
| 版本 | 日期 | 变更 |
|------|------|------|
| 2.0.0 | 2026-07-01 | AI 化重构 8 阶段完成：①抽 10 AI 组件（FlowNode/FlowArrow/AgentCard/SkillCard/LimitationCard/Highlight/SectionTag/CodeBlock/PulseDot/KpiCard）至 src/client/components/ai/；②新增 design-tokens.css + ConfigProvider darkAlgorithm 注入；③Layout.tsx 深色重做（KM Portal 标题 + AI 在线 PulseDot + Setting 图标）；④Dashboard 流程图版（4 KpiCard + Agent 调用流 3 段 SVG + 时间线 CodeBlock）；⑤KM Studio 单页流程图版（4 阶段 FlowNode + AgentCard 包裹 Tabs）；⑥TokenManage 健康度仪表 + SkillGen 紫边 SkillCard + Progress 条 + KBBrowser CodeBlock 内容预览 + DocEditor AI 助手标签；⑦Stats ECharts 替换 BarChartCard（5 色渐变柱状图） + ApiDocs 深色 CodeBlock + 复制按钮；⑧package.json 加 echarts 依赖（+1.3MB / +419KB gzipped）；npm run lint 0 错误 + typecheck 0 错误 + 189 测试全绿 + build:all EXIT 0 |
