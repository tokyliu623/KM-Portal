...
router.post('/init', async (req, res, next) => {
    try {
        ...
        try {
            const kbInfo = await kmApiClient.getKBInfo(String(kbIdNum), tokenStr);
            const response = {
                valid: true,
                kbInfo: {
...
router.post('/diagnose', async (req, res, next) => {
    try {
        ...
        const kmApiClient = req.app.locals.kmApiClient;
        try {
            const treeData = await kmApiClient.getKBTree(String(kbIdNum), tokenStr);
...
router.post('/generate', async (req, res, next) => {
    try {
        ...
        const response = { jobId, status: 'pending', products: initialProducts };
        res.json({ success: true, data: response });
    }
    catch (error) {
        next(error);
